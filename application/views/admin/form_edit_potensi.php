   ...
   <div class="col-md-4">
     ...
     <div class="col-md-12">
       <div class="panel panel-default">
          ...
          <form class="form-group" method="post" action="<?php echo site_url('admin/kelola_potensi/update');?>">
           <div class="panel-body">
              <label>Nama Desa</label>
               <input type="hidden" class="form-control" name="kode_desa" value="<?php echo $kode_desa;?>">
                <input class="form-control" name="desa" value="<?php echo $tempat;?>" readonly>
            <?php  foreach ($potensi->result() as $row):?>
             
              <input type="hidden" class="form-control" name="kode_potensi" value="<?php echo $row->kode_potensi;?>">
               
              <label>Nama Potensi</label>
              <input required  class="form-control" value="<?php echo ($row->nama_potensi);?>" name="nama_potensi">
              ...
              <?php $no=1;foreach ($potensi->result() as $row):?>
                  <tr title="<?php echo "$row->nama_potensi";?>">
                    ...
                    <td class="text-center">
                     <a id="edit" title="Edit Data potensi" class="btn btn-warning btn-sm" href="<?php echo site_url("admin/kelola_potensi/form_edit/$row->kode_potensi");?>"><i class="glyphicon glyphicon-edit"></i> </a>
                     <a title="Hapus Data potensi" class="btn- btn-danger btn-sm" href="<?php echo site_url("admin/kelola_potensi/hapus/$row->kode_potensi");?>"><i class="glyphicon glyphicon-remove"></i> </a>
                    </td>
                  </tr>
              <?php endforeach;?>    
              </tbody>
